...
float Parser::variable, Parser::variable2;

Parser::Parser(System::String ^str, float var, float var2)
{
	string strstring;
	for (int i = 0; i < str->Length; i++)
		strstring += (char) str[i];

	Parser_default(&strstring, var, var2);
}

Parser::Parser(string *tmp, float var, float var2)
{
	Parser_default(tmp, var, var2);
}
...
void Parser::Parser_default(string *tmp, float var, float var2)
{
	//**
	//ofstream out("out.txt", ios::app);
	//out << "\n*******\ni am in parser constructor String\n******\n" << "str = \"" << tmp << "\"\n" << endl;
	//**
...
float Parser::ConvertToFloat()
{
	...
	else if (limit.first == limit.second && ParserStr[limit.first] == 'y')
	{
		return variable2;
	}
	else
	{
		...
		for (int itmp = limit.first; itmp <= limit.second; itmp++)
		{
			...
			{
				CharAfterComma++;
				answer = answer + (ParserStr[itmp] - 48) / pow(10, CharAfterComma);
			}
		}
		return answer;
	}
